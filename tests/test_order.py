...
class TestOrderAndNavigation:  # Класс для тестов заказа и навигации по логотипам
    ...
    @pytest.mark.parametrize("name, last_name, address, metro, phone, date, period, comment", OrderData.ORDER_DATA_LIST) # Параьетризация по всем наборам данных для заказа
    def test_order_positive_flow_top_button(self, driver, name, last_name, address, metro, phone, date, period, comment):  # Тест заказа через верхнюю кнопку
    ...
    @pytest.mark.parametrize("name, last_name, address, metro, phone, date, period, comment", OrderData.ORDER_DATA_LIST) # Параьетризация по всем наборам данных для заказа
    def test_order_positive_flow_bottom_button(self, driver, name, last_name, address, metro, phone, date, period, comment):  # Тест заказа через нижнюю кнопку
        main_page = MainPage(driver)  # Создаем объект главной страницы
        ...
        order_page.fill_personal_info(name, last_name, address, metro, phone)  # Заполняем Личные данные
        order_page.fill_rental_info(date, period, comment)  # Заполняем Данные об аренде
        assert order_page.check_success_popup_is_displayed()  # Проверяем успешность создания заказа
